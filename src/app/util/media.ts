...
export async function findSignedURL(key: string, isAttachment: boolean = true): Promise<string> {
  ...
  for (const e1 of url.filter(e => (Date.now() / 1000) > e.expires)) await db.collection<DBSCacheItem>("scache").deleteOne({_id: e1._id, attachment: isAttachment || false});
